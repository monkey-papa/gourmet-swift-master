...
//        webSocketServer.sendToAllClient("新订单到达: " + orders.getId());
...
//    }
    ...
    public void userCancelById(Long id) throws Exception {
        ...
        Orders orders = new Orders();
        orders.setId(ordersDB.getId());
        orders.setStatus(Orders.CANCELLED); // 假设 CANCELLED 为已取消状态的枚举值
        orders.setCancelReason("用户取消");
        orders.setCancelTime(LocalDateTime.now());
...
//    }
    ...
    public void rejection(OrdersRejectionDTO ordersRejectionDTO) throws Exception {
        ...
        // 无需进行支付状态检查和退款，直接更新订单为拒绝状态
        Orders orders = new Orders();
        orders.setId(ordersDB.getId());
        orders.setStatus(Orders.CANCELLED); // 设置订单状态为已取消
        orders.setRejectionReason(ordersRejectionDTO.getRejectionReason()); // 设置拒绝原因
        orders.setCancelTime(LocalDateTime.now()); // 设置取消时间

        // 更新订单状态
        orderMapper.update(orders);

        log.info("订单已拒绝，订单号：{}, 原因：{}", ordersDB.getNumber(), ordersRejectionDTO.getRejectionReason());
    }
...
//    }
    ...
    public void cancel(OrdersCancelDTO ordersCancelDTO) throws Exception {
        ...
        //订单状态 1待付款, 2待接单 可以取消
        if (ordersDB.getStatus() > 2) {
            throw new OrderBusinessException("订单当前状态不允许取消");
        }
        ...
        // 记录取消操作日志
        log.info("订单取消成功：订单ID {}, 取消原因 {}", ordersCancelDTO.getId(), ordersCancelDTO.getCancelReason());
    }
